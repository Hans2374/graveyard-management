...
const rows = [
  createData(1, 'EMPLOYEE01', false),
  createData(2, 'EMPLOYEE02', false),
  createData(3, 'EMPLOYEE03', false),
  createData(4, 'EMPLOYEE04', false),
  ...
  return (
    <TableContainer component={Paper} sx={{ width: '1080px' }}>
      <Table aria-label="simple table">
        ...
        <TableBody>
          {rows.map((row, index) => (
            <TableRow key={row.tennhanvien} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
              <TableCell align="left" scope="row">
                {row.stt}
              </TableCell>
              <TableCell align="left">{row.tennhanvien}</TableCell>
